...
resource "aws_s3_bucket_object" "lambda_zip" {
  ...
  etag = local.source_zipped ? md5(file(var.source_dir)) : filemd5(data.archive_file.files[0].output_path)
}
...
resource "aws_lambda_function" "function" {
  ...
  source_code_hash = local.source_zipped ? base64sha256(file(var.source_dir)): data.archive_file.files[0].output_base64sha256
